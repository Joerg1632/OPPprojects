...
int main(int argc, char* argv[]) {
    ...
    #pragma omp parallel
    {
        #pragma omp for schedule(dynamic, atoi(argv[1])) reduction(+:normb)
        for (int i = 0; i < N; ++i) {
            normb += b[i] * b[i];
        }
        ...
        do
        {
            ...
            #pragma omp for schedule(dynamic, atoi(argv[1])) reduction(+:Arr)
            for (int i = 0; i < N; ++i) {
                Arr += Ar[i] * residual[i];
            }

            #pragma omp for schedule(dynamic, atoi(argv[1])) reduction(+:ArAr)
            for (int i = 0; i < N; ++i) {
                ArAr += Ar[i] * Ar[i];
            }
